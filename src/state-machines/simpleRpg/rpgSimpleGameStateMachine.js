...
const enqueueTextOptionGoToSteps = assign({
  stepsQueue: (ctx, _) => {
    const _goToSteps =
      ctx.constants.steps[ctx.stepsQueue[0]].options[ctx.currentTextOption]
        .goTo || [];
    return [...ctx.stepsQueue, ..._goToSteps];
  }
});
...
const selectPrevTextOption = assign({
  currentTextOption: (ctx, _) => {
    ...
    const _newTextOption =
      ctx.currentTextOption <= 0
        ? _options.length - 1
        : ctx.currentTextOption - 1;
...
const selectNextTextOption = assign({
  currentTextOption: (ctx, _) => {
    const _options = ctx.constants.steps[ctx.stepsQueue[0]].options;
    const _newTextOption =
      ctx.currentTextOption >= _options.length - 1
        ? 0
        : ctx.currentTextOption + 1;
    return _newTextOption;
  }
...
const customStepExists = (ctx, event) => {
  return (
    ...
    ctx.constants.steps &&
    event &&
    ctx.constants.steps[event.step] !== undefined
  );
};
...
const isBattle = ctx => {
  ...
  return (
    ctx.constants &&
    ctx.constants.steps &&
    ctx.constants.steps[currentStep] &&
    ctx.constants.steps[currentStep].type === "battle"
  );
};
...
const isAudio = ctx => {
  ...
  return (
    ...
    ctx.constants.steps &&
    ctx.constants.steps[currentStep] &&
    ctx.constants.steps[currentStep].type === "audio"
  );
};
...
const isText = ctx => {
  ...
  return (
    ...
    ctx.constants.steps &&
    ctx.constants.steps[currentStep] &&
    ctx.constants.steps[currentStep].type === "text"
  );
};
...
const isTextOption = ctx => {
  ...
  return (
    ...
    ctx.constants.steps &&
    ctx.constants.steps[currentStep] &&
    ctx.constants.steps[currentStep].type === "textOption"
  );
};
...
const isBackgroundChange = ctx => {
  ...
  return (
    ...
    ctx.constants.steps &&
    ctx.constants.steps[currentStep] &&
    ctx.constants.steps[currentStep].type === "backgroundChange"
  );
};
    ...
    playBattle: {
      invoke: {
        ...
        data: {
          ...
          enemy: context => {
            const enemyId =
              context.constants.steps[context.stepsQueue[0]].enemy;
