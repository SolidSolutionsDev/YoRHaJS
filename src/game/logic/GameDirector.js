...
export class GameDirector extends Component {
  ...
  ballCollided = e => {
    ...
    // console.log( e );
    const _collidedBody =
      _originalEvent.contact.bi === _physicsRepresentation.body
    ...
    if (_collidedBody.instance.collides) {
      ...
      let positionToAttach = _mainBall.bodiesInContact
        .filter(
          candidateElementToAttach =>
            candidateElementToAttach !== _collidedBody.instance &&
            candidateElementToAttach.attaches &&
            !candidateElementToAttach.collides
        )
        .forEach(candidateElementToAttach => {
          ...
          if (_nearestBall === undefined) {
            _nearestBall = candidateElementToAttach;
            return;
          } else {
            const _distanceCandidateBall = candidateElementToAttach.mesh
              .getWorldPosition()
              .distanceTo(_mesh.getWorldPosition());
            const _distanceActualNearestBall = _nearestBall.mesh
              .getWorldPosition()
              .distanceTo(_mesh.getWorldPosition());
            _nearestBall =
              _distanceCandidateBall < _distanceActualNearestBall
                ? candidateElementToAttach
                : _nearestBall;
          }
        });
