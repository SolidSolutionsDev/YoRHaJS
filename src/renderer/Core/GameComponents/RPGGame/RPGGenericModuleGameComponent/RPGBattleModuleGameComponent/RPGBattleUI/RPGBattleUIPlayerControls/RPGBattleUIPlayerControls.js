...
export class RPGBattleUIPlayerControls extends React.Component {
  ...
  updateSelectedMenuCommand = time => {
    ...
    if (activeLeft || activeRight || activeUp || activeDown) {
      ...
      const totalTimesMenuChangedSinceActivated =
        this.activeTimesMenuMoved * this.menuChangeOptionInterval;
      const timePassedFromLastMenuMovement =
        time -
        (this.startActiveMenuMovementTime +
          totalTimesMenuChangedSinceActivated);
      const menuMovementsToDoNow = Math.floor(
        timePassedFromLastMenuMovement / this.menuChangeOptionInterval
      );
      for (
        let menuChangeIndex = 0;
        menuChangeIndex <= menuMovementsToDoNow;
        menuChangeIndex++
      ) {
            ...
            <ul class="battle-menu">
                ${Object.values(this.availableCommands)
                  .map((commandId, index) => {
                    ...
                    return `<li
                                class="${`battle-menu-option ${
                                  currentActive
                                    ? "battle-menu-option-selected"
                                    : ""
                                }`}"
                            >
                                ${attackData.label}
  ...
  attachMenu = menuDiv => {
    ...
    this.cssGameComponent = gameObject.getComponent("CSSLabelTo3D");
