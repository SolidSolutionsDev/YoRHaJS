...
export class SimpleRPGBattle extends React.Component {
  ...
  advance = () => {
    ...
    if (this.state.active) {
      const commandToSend =
        this.state.gameCurrent.value === "playBattle" ? "INPUT" : "NEXT_STEP";
        ...
        if (game.service && this.battleService !== battleMachineService) {
          ...
          if (battleMachineService) {
            ...
            this.updateMessagesText(
              this.battleService.state.context.statusMessage,
              current.context
            );
          }
          console.log(this.battleService);
          const a = this.battleService
            ? this.battleService.onTransition(newBattleState => {
                ...
                if (newBattleState.context.statusMessage) {
                  this.updateMessagesText(
                    newBattleState.context.statusMessage,
                    current.context
                  );
                }
                const turnHasChanged =
                  this.state.battleCtx &&
                  this.state.battleCtx.currentTurn !==
                    newBattleState.context.currentTurn &&
                  newBattleState.context.currentTurn;
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (this.state.battleCtx) {
      ...
      if (
        this.persistentObjects.length &&
        this.state.battleCtx.diceValue &&
        this.state.battleCtx.diceValue !== prevState.battleCtx.diceValue
      ) {
  ...
  activateTextElements = () => {
    if (this.persistentObjects.length) {
      this.persistentObjects.forEach(persistentObjectId =>
        this.showObject(persistentObjectId)
      );
    } else {
  ...
  instantiateTextualGraphicElements = () => {
    ...
    this.instantiateTextualGraphicElement(
      "player",
      "text",
      this.getText("player")
    );
    ...
    this.instantiateTextualGraphicElement(
      "gameMessages",
      "text",
      this.getText("gameMessages")
    );
  };

  instantiateTextualGraphicElement = (
    battleEntity = "player",
    graphicElement = "counter",
    value = ""
  ) => {
    console.log(this);
    const battleConstants = this.gameService.context.constants.battle;
    ...
    const graphicElementPosition =
      battleConstants.graphicElementsPositions[battleEntity][graphicElement];
    const graphicElementScale =
      battleConstants.graphicElementsScales[battleEntity][graphicElement];
    const { scene } = this.props.availableComponent;
    ...
    scene.enqueueAction(
      instantiateFromPrefab(
        ...
        {
          position: {
            x: graphicElementPosition[0],
            y: graphicElementPosition[1],
            z: graphicElementPosition[2]
          },
          scale: {
            x: graphicElementScale,
            y: graphicElementScale,
            z: graphicElementScale
          }
        },
  ...
  destroyModel = (modelType = "player") => {
    ...
    scene.enqueueAction(
      destroyGameObjectById(
        this.graphicElements[modelType].model,
        this.props.gameObject.id
      )
    );
  };
  ...
  cleanBattle = () => {
    ...
    this.persistentObjects.forEach(persistentObjectId =>
      this.hideObject(persistentObjectId)
    );
  ...
  hideObject = objectId => {
    const { scene } = this.props.availableComponent;
    scene.enqueueAction(updateGameObject(objectId, { enabled: false }));
  };

  showObject = objectId => {
    const { scene } = this.props.availableComponent;
    scene.enqueueAction(updateGameObject(objectId, { enabled: true }));
  };
  ...
  render() {
    ...
    const battleUI =
      gameCurrent && gameCurrent.value === "playBattle" ? (
        <div>
          BATTLE!
          <div>{gameCurrent.value + " -> " + battleCurrent.value}</div>
          <div
            style={{
              color: battleCtx.currentTurn === "player" ? "green" : "red"
            }}
          >
            Dice:{battleCtx.diceValue}
          </div>
          <div>Damage Done {battleCtx.damageDone}</div>
          <div>
            Player{" "}
            {`${battleCtx.player.name} hp:${battleCtx.player.hp} defense:${battleCtx.player.defense} `}
          </div>
          <div>
            Enemy{" "}
            {`${battleCtx.enemy.name} hp:${battleCtx.enemy.hp} defense:${battleCtx.enemy.defense} `}
          </div>
        </div>
      ) : null;
    return (
