...
export class EnemyMovementControls extends React.Component {
  ...
  getShooter = () => {
    const { gameObject, availableComponent } = this.props;
    const { scene } = availableComponent;
    const shooterById = gameObject.getChildGameObjectByTag(
      "playerShooter",
      scene
    );
    return shooterById;
  ...
  updateFollowPlayerEnemy = (time, deltaTime) => {
    ...
    // get angle to rotate
    const maxRotateValue = 0.05;
    const rotationToLookAtPlayer = Math.asin(
      vPlayerPositionRelativeToLocalEnemyPositionAndRotation.x
    );
    const rotateAbsoluteValue = Math.min(
      Math.abs(rotationToLookAtPlayer),
      maxRotateValue
    );
    const orientedRotationValue =
      -Math.sign(rotationToLookAtPlayer) * rotateAbsoluteValue;

    transform.rotateZ(orientedRotationValue);
  };

  updateAutoRotateEnemy = (time, deltaTime) => {
    this.props.transform.rotation.z +=
      (this.rotationSpeed * (0.02 * deltaTime)) / 10;
  };
