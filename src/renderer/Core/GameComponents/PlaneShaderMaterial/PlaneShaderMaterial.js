...
export class PlaneShaderMaterial extends React.Component {
  ...
  initExplode = () => {
    ...
    if (explode) {
      ...
      this.geometry = availableService.geometry.generateExplodableBufferGeometryFromGeometry(
        this.geometry,
        explode.directionFunction,
        explode.maxEdgeLength,
        explode.tessellateIterations
      );
  ...
  updateUniforms = time => {
    ...
    // console.log(audioObject);
    if (explode) {
      const audioMultiplier = explode.audioTag
        ? audio.availableAudio[audioTag].analyser.getAverageFrequency() / 255
        : 1.0;
      this.uniforms.amplitude.value =
        audioMultiplier * Math.sin(time * explode.timeScale) * explode.distance;
    }
  };
