...
export class GeometryUtilsService extends Component {
  ...
  generateExplodableBufferGeometryFromGeometry = (
    geometry,
    displacementFunctionId,
    maxEdgeLength,
    tessellateIterations
  ) => {
    ...
    for (let faceId = 0; faceId < numFaces; faceId++) {
      ...
      const faceDisplacement = this.explosionDirectionFunctions[
        displacementFunctionId
      ]
        ? this.explosionDirectionFunctions[displacementFunctionId]()
        : this.explosionDirectionFunctions.default();
