...
export class SimpleRPGStateMachineService extends Component {
  ...
  registerTransitionListener = () => {
    this.stateMachines.game.service.onTransition(gameState => {
      ...
      const transitionedToANewBattleState =
        this.battleService !==
        this.stateMachines.game.service.children.get("battle");
      if (transitionedToANewBattleState) {
        this.battleService = this.stateMachines.game.service.children.get(
          "battle"
        );
        const a = this.battleService
          ? this.battleService.onTransition(state => {
              ...
              this.setState({
                battleCurrent: state,
                battleValue: state.value,
                battleCtx: state.context
              });
            })
          : null;
      }
      this.setState({
        gameCurrent: gameState,
        gameValue: gameState.value,
        gameCtx: gameState.context
      });
    });
  };
  ...
  renderDebugMachine() {
    ...
    return (
      <div key={"debug-battle"} className={"debug-battle"}>
  ...
  render() {
    return <div>{this.debug ? this.renderDebugMachine() : null}</div>;
  }
}
