...
export const mainReducer = (state = initialScene, action) => {
  ...
  switch (action.type) {
    ...
    case "REGISTER_CAMERA":
      ...
      Object.keys(temp.scenes)
          .forEach(
              (sceneId)=> {
                temp.scenes[sceneId].camera.main = temp.scenes[sceneId].camera.main ? temp.scenes[sceneId].camera.main: action.cameraId;
              }
          );
    ...
    case "SET_MAIN_CAMERA":
      ...
      temp.scenes = {...state.scenes ,[action.sceneId]:{...state.scenes[action.sceneId], camera: temp.camera }};
